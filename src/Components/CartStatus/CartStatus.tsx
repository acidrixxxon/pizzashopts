...
const CartStatus = () => {
  ...
  const [countShaking, setCountShaking] = React.useState<boolean>(false);
  ...
  React.useEffect(() => {
    if (params.pathname === '/cart') setVisibleList(false);
  }, [params.pathname]);

  return (
    <>
      <div
        className={visibleList ? 'cartstatus active' : 'cartstatus'}
        onClick={params.pathname !== '/cart' ? () => setVisibleList((state) => !state) : undefined}
        ref={refEl}>
        ...
        <span className='cartstatus__price'>
          {cart === null ? 0 : cart.totalCost < 300 && cart.totalItems > 0 ? cart.totalCost + 40 : cart.totalCost}
          .00 грн
        </span>
          ...
          {cart.totalItems === 0 && (
            <div className='cartStatus__empty'>
              <div className='cartStatus__promo'>
                ...
                <div className='cartStatus__promoText'>
                  Подвійна моцарела
                  <br /> від 20грн
                </div>
              </div>
              <svg
                className={cart.totalItems === 0 ? 'cartStatus__emptyIcon shake__animation' : 'cartStatus__emptyIcon'}
          ...
          {cart.totalItems > 0 && (
            <>
              ...
              <AnimatePresence>
                <ul className='cartStatus__itemList'>
                  {cart.items.map((item: any) => (
                    <CartStatusItem item={item} key={item._id} />
                  ))}
              ...
              <div className='cartStatus__footer'>
                <Link to='/cart' className='cartStatus__createBtn'>
                  Оформити
      ...
      <Link
        className={params.pathname === '/cart' ? 'cartstatus cartstatus__mobile cartstatus__hidden' : 'cartstatus cartstatus__mobile'}
        to='/cart'>
        <span className={countShaking ? 'cartstatus__count shake__animation' : 'cartstatus__count'}>
          {cart === null ? '00' : cart.totalItems === 0 ? '00' : cart?.totalItems < 10 ? `0${cart?.totalItems}` : cart?.totalItems}
          <AiOutlineShoppingCart />
        ...
        <span className='cartstatus__orderBtn'>Замовити</span>
      </Link>
    </>
  );
};
