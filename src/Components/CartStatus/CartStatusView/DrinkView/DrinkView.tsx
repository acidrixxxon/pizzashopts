...
import './_DrinkView.scss'

interface ComponentProps {
  item: IDrinkInCart
}

const DrinkView:React.FC<ComponentProps> = ({ item }) => {
  const [ animation,setAnimation ] = React.useState<'scaleup__animation' | 'scaledown__animation' | null>(null)

  const { actions: { changeItemQty },state: { cart } } = React.useContext(Context1)
  ...
  return (
    <li className='drinkView'>
      <div className="cartStatus__header">
        <h4 className="cartStatus__title">{item.title} ({item.size})</h4>
      </div>
