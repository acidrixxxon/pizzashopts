      ...
      <div className='order__data'>
        ...
        {item.ingridients && (
          <div className='order__itemFooter'>
            <p>
              {item.ingridients.map((ingridient, index: number) => {
                return `${index === 0 ? ingridient.ingridientId.title : ingridient.ingridientId.title.toLowerCase()}${
                  item.ingridients !== undefined && index + 1 !== item.ingridients.length ? ', ' : ''
                }`;
              })}
            </p>
          </div>
