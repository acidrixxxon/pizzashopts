...
const SideList:React.FC = () => {
    ...
    const { state: { sort: { category,sort },socket}} = React.useContext(Context1)
        ...
        <>
            {loading ? <Skeleton /> :         
                <div id="side__list"  className='list'>
                    {sort === 0 && categories !== null ? categories.map((cat) => {
                        if (cat.products.length > 0 && cat.products !== null) {
                            return (
                                <div className='side__category category'>
                                    ...
                                    <ItemsList>
                                        {cat.products.map((prod) => <SideComponent item={prod} key={prod._id}/>)}
                                    </ItemsList>
                                </div>
                            )
                        }
