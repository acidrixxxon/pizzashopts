      ...
      <div id="dineinform">
        <div className="dineinform__group">
          ...
          <div className="dineinform__list" onClick={showCityListHandler}>
            ...
            <svg width="10" className={showCity ? 'rotated' : ''} height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path style={customerData.city ? {fill: '#4f4f4f'} : {}} d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z" fill="#2C2C2C"></path></svg>
          
            {showCityList &&
              <ul className='dineinform__itemList'>
                {restaurants.map(item => <li className="dineinform__item" key={item.id} onClick={() => setCustomerData({target: {name: 'city',value: {id: item.id,name: item.name}}})}>{item.name}</li>)}
              </ul>
            }
          </div>
          ...
          <>
            <div className="dineinform__group">
              ...
              <div className="dineinform__list" onClick={showRestaurantsListHandler}>
                ...
                <svg width="10" className={showRestaurants ? 'rotated' : ''} height="6" viewBox="0 0 10 6" fill="none" xmlns="http://www.w3.org/2000/svg"><path style={customerData.restaurant ? {fill: '#4f4f4f'} : {}} d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z" fill="#2C2C2C"></path></svg>

                {showRestaurants && 
                  <ul className="dineinform__itemList">
                    {restaurants?.find(item => customerData.city !== undefined && customerData.city !== null ? item.id === customerData.city.id : null)?.restaurants.map(item => <li className="dineinform__item" key={item.id} onClick={() => setCustomerData({target: {name: 'restaurant',value: item}})}>{item.address}</li>)}
                  </ul>}
              </div>
