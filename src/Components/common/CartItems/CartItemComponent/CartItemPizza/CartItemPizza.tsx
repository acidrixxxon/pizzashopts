...
const CartItemPizza: React.FC<ComponentProps> = ({ item }) => {
  ...
  const { removeFromCart, changeItemQty } = getCartActions(dispatch, state);
    ...
    <li id='cartitem'>
      ...
      <div className='cartitem__description'>
        <div className='cartitem__header'>
          <span className='cartitem__title'>{item.fulltitle}</span>

          <AiOutlineClose onClick={() => removeFromCart(item)} />
        </div>

        {item.ingridients !== undefined && (
          <div className='cartitem__body'>
            {item.ingridients.map(({ ingridientId: { title, _id }, qty }, index) => {
              return (
                <span key={_id}>
                  {title}
                  {qty > 1 && '(Подвіна порція)'}
                  {item.ingridients !== undefined && item.ingridients.length > index + 1 ? ', ' : null}
                </span>
              );
            })}
          </div>
